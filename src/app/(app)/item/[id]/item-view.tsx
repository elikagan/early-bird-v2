...
export default function ItemView({ initialItem }: { initialItem: ItemDetail }) {
  ...
  // Public item fields come in via initialItem (server-rendered).
  // This effect only fetches the per-user slice and the dealer's own
  // inquiries list.
  useEffect(() => {
    async function load() {
      if (!user) {
        const { hasAnonFavorite } = await import("@/lib/anon-favorites");
        setIsFav(hasAnonFavorite(id));
        return;
      }
      const meRes = await apiFetch(`/api/items/${id}/me`);
      if (meRes.ok) {
        const meData: Partial<ItemDetail> = await meRes.json();
        setItem((prev) => ({ ...prev, ...meData }));
        setIsFav(!!meData.is_favorited);
        setFavId(meData.favorite_id || null);
      }
      if (initialItem.dealer_user_id === user.id) {
        const inqRes = await apiFetch(`/api/items/${id}/inquiries`);
        if (inqRes.ok) setInquiries(await inqRes.json());
      }
    }
    load();
  }, [id, user, initialItem.dealer_user_id]);
  ...
  const sendInquiry = useCallback(async () => {
    ...
    if (res.ok) {
      if (user) {
        ...
        setIsFav(true);
        setItem((prev) => ({ ...prev, my_inquiry_status: "open" }));
      } else {
