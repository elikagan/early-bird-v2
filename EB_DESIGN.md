...
Buyer: name, avatar, preferences. Dealer: adds business name, optional Instagram handle (stored as `@handle` only — the app renders the URL), accepted payment methods (checkboxes only — no stored handles), booth settings. Instagram surfaces on `item-detail-buyer` wherever the dealer name/avatar appears so buyers can tap through to the dealer's feed. Phase 2 must add an optional `instagram` column to the `dealers` table.
